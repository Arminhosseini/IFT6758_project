...
def tidy_data(path):
    try:
        ...
        df.loc[:, "period"] = df["allPlays"].apply(lambda x: x.get("about")["period"])
        df.loc[:, "periodType"] = df["allPlays"].apply(lambda x: x.get("about")["periodType"])
        df.loc[:, "periodTime"] = df["allPlays"].apply(lambda x: x.get("about")["periodTime"])
        df.loc[:, "periodTimeRemaining"] = df["allPlays"].apply(lambda x: x.get("about")["periodTimeRemaining"])
        df.loc[:, "dateTime"] = df["allPlays"].apply(lambda x: x.get("about")["dateTime"])
        df.loc[:, "teamId"] = df["allPlays"].apply(lambda x: x.get("team")["id"])
        df.loc[:, "teamName"] = df["allPlays"].apply(lambda x: x.get("team")["name"])
        df.loc[:, "teamTriCode"] = df["allPlays"].apply(lambda x: x.get("team")["triCode"])
        df.loc[:, "eventType"] = df["allPlays"].apply(lambda x: x.get("result")["event"])
